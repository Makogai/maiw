...
| 2026-07-30 | Android Play automation uses Fastlane + GitHub Actions with branch flow `preprd -> beta` and `prod -> production draft` | Need repeatable testing + release staging without local manual Play Console uploads | Release path remains DriveBay-only; CI reconstructs keystore + Play JSON from repo secrets, builds the AAB, then runs `fastlane beta` or `fastlane production` | `.github/workflows/play-beta.yml`, `.github/workflows/play-prod.yml`, `android/fastlane/Fastfile`, `docs/development/play-release.md`, **Jira: KAN-72** |
