...
| `auth_repository.dart` | `/auth/login`; `/auth/social/{provider}`; `/auth/register`; `/auth/verify-email`; `/auth/verification/resend`; `/auth/forgot-password`; `/auth/reset-password`; `/auth/account/restore`; `/auth/me`; `/auth/logout` | POST×8+; GET; POST | Yes — includes social login (`google`\|`facebook`\|`apple`) + password reset (**Jira: KAN-56**) | `lib/repositories/auth_repository.dart` |
