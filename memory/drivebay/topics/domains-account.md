...
| `PaddlePaymentGateway` | Paddle Billing transaction checkout; webhooks `transaction.completed`/`transaction.paid`; web sync via POST `/billing/paddle-checkout/complete`; mobile Sanctum sync via POST `/api/v1/billing/paddle/complete` + Vue `DriveBayNative` bridge (**Jira: KAN-111**) — `Domains/Billing/Gateways/PaddlePaymentGateway.php` |
...
| Class | Role |
|---|---|
| `ViewingAppointmentService` | `book()` (validates not-own-listing, active listing, slot availability inside a DB transaction with a re-check to close a race window), `cancel()`, `reschedule()`, presenters for upcoming/past lists; `reschedule()` keeps the same appointment row, re-checks slot availability before and inside the transaction, resets reminder history, and exposes presenter `can_reschedule` — `Domains/Viewing/Services/ViewingAppointmentService.php:22,53,87,115,310` |
...
| `ViewingNotificationService` | Writes `Notification` rows directly (booked/cancelled/reminder/rescheduled); includes `clearReminderHistory()` and dual-party `viewing.rescheduled` notifications — `Domains/Viewing/Services/ViewingNotificationService.php:16,68,139,140,185` |
...
| `ViewingAppointmentPolicy` | `book()` requires verified email + not own listing; `cancel()` requires being buyer or seller; `reschedule()` is buyer-only for future confirmed appointments — `Domains/Viewing/Policies/ViewingAppointmentPolicy.php:11,20,29` |
...
**KAN-35 shipped behavior (app HEAD `727abc9`)**: buyer reschedule keeps the same
`ListingViewingAppointment` row instead of
...
only for the buyer in presenter payloads. Verified by backend tests
`tests/Feature/ApiViewingTest.php` and `tests/Feature/ViewingSchedulingTest.php` before the
feature was pushed.
